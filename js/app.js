...
var CELULAR_EMPRESA = "5581981317866";
...
cardapio.eventos = {
  init: () => {
    ...
    cardapio.metodos.carregarBotaoAtendimento();
  },
};
...
cardapio.metodos = {
  ...
  obterItensCardapio: (categoria = "burgers", vermais = false) => {
    ...
    $.each(filtro, (i, e) => {
      ...
      //botaão ver mais foi clicado (12 itens)
      if (vermais && i >= 8 && i < 12) {
  ...
  // Carrega o botão de atendimento
  carregarBotaoAtendimento: () => {
    var texto = `Olá! gostaria de entrar em *contato*`;

    let encode = encodeURI(texto);
    let URL = `https://wa.me/${CELULAR_EMPRESA}?text=${encode}`;

    $("#btnFaleConosco").attr("href", URL);
    $("#btnFaleConosco-1").attr("href", URL);
  },
  ...
  mensagem: (texto, cor = "red", tempo = 3500) => {
    ...
    let msg = `<div id="msg-${id}" class="animated fadeInDonw toast ${cor}">${texto}</div>`;
